...
##     HAM(Genuine image, just to rhyme with spam) images.                 ##
...
def calc_entropy_noise(img):
    ...
    f = img_gray[230:290, 220:320]
    noisy = f + 0.4 * f.std() * np.random.random(f.shape)
    # Calculate the histogram
    x = itemfreq(noisy.ravel())
    # Normalize the histogram
    hist = x[:, 1]/sum(x[:, 1])
    noise_entropy = -sum([p * math.log(p, 2) for p in hist if p != 0])
...
c = csv.writer(open("ham_out_fs7.csv",'w',encoding='utf8',newline=''))
...
for file in listing:
    ...
    if not (file.startswith('.')):
        ...
        #c.writerow(('Image_name','SNR','Entropy of noise','Entropy of LBP','Entropy of HOG','Variance1','Variance2','Variance3'))
        c.writerow((file,snr,noise_entropy,lbp_entropy,entropy_hog,var[0],var[1],var[2]))
